...
def ntransform(image, cfg):
    ...
    if "normalize" in cfg.INPUT.TRANSFORMS:
        ...
        to_tensor += [normalize]
    to_tensor = T.Compose(to_tensor)
    return to_tensor(image)


def xloss_compute(text_features, imf_i, cfimf_i):
    pos = torch.cosine_similarity(text_features, imf_i, dim=1)
    neg = torch.cosine_similarity(text_features, cfimf_i, dim=1)

    logit = torch.stack((pos, neg), 1) # [b, 2]
    softmax_logit = nn.functional.softmax(logit, 1) #[b, 2]
    xloss = - torch.log(softmax_logit[:, 0])
